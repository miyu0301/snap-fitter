...
type ValidateUserObject = {
  [K in keyof User]: K extends "goal_id" | "level_id"
    ? User[K] extends number
      ...
      : never
    : K extends "gender"
    ? User[K] extends 1 | 2 | 3
      ? User[K]
      : never
    : K extends "birthday" | "height" | "weight"
    ? User[K] extends string | number
      ? User[K] extends "" | undefined
        ? never
        : User[K]
      : never
    : never;
...
export const UserProvider: React.FC = ({ children }) => {
  ...
  const [dbUser, setDBUser] = useState<UserInfo>(initialDbUser);
